 ...
 * Attributes are compared based on the depth order of the attribute. this attribute 
 * will be smaller if the depth corresponding to it is less than the compared one. 
 * return 0 if depth is same, 1 if current attribute depth is higher, -1 if current 
 * attribute depth is less.
 * 
 * @author biswa
 * 
 */
public abstract class Attribute implements Comparable<Attribute>,
	...
	/**
	 * Defines constant invalid minor.
	 */
	private static final int INVALID_MINOR = Integer.MIN_VALUE;
	/**
	 * Name of the attribute can not be null or space.
	 */
	final private String name;
	
	/**
	 * minor is used to identify multi valued substance
	 */
	...
	/**
	 * Tells whether this attribute is private / not
	 */
	transient private boolean propagate = false;
	
	/**
	 * this attribute is dependent on following attribute list.
	 */
	final transient private Collection<Attribute> dependsOnList = new HashSet<Attribute>();
	/**
	 * Attributes dependent on this attribute in the current container. Container manages this. Dont worry.
	 * Do not add anything ore remove anything from here.
	 */
	...
	protected Attribute(String name,int minor) {
		assert name!= null && name.trim().length()>0 :
			"Can not create Attribute with name null or empty String";
		this.name = name.trim();
		this.minor=minor;
	}
	/**
	...
	/**Returns whether this attribute requires any storage space
	 * 
	 * @return boolean
	 */
	...
	/**Method tells should this attribute be initialized prior to insert
	 * 
	 * @return
	 */
	...
	/**This method returns vertical propagation property of any attribute.
	 * 
	 * @return boolean
	 */
	...
	/**This method returns whether this attribute is a channel attribute
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Tells whether this attribute is state less.
	 */
	 ...
	 * creation. Guest Attribute should not be null. Adding
	 * duplicate attribute will result no operation.
	 * 
	 * @param attribute
	 *            Attribute
	 */
	 ...
	 * dependent on any other attribute.
	 * 
	 * @return Attribute[]
	 */
	...
	final int getDependencyDepth() {
		if(dependencyDepth==-1){
			...
			for(Attribute attribute:dependsOnList){
				dependencyDepth=dependencyDepth+attribute.getDependencyDepth();
			}
		}
	...
	final void prepare(){
		if(dependsOn().length>0){
			...
			for(Attribute oneDependee:dependsOn()){
				Attribute registeredAttribute = oneDependee.getRegisteredAttribute(); 
	...
	protected Object getValue(ContainerEntry containerEntry,String name){
		return containerEntry.getSubstance(containerEntry.getContainer().getAttributeByName(name)).getValue();
	}
	...
	final protected Object getStatic(String attributeName){
		...
		return container.getStatic(container.getAttributeByName(attributeName)).getValue();
	}
